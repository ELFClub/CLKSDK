...
#import "CLKUserModel.h"

@interface CLKSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(CLKCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(CLKCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，CLKSDKDefines中有声明
 */
- (void)setLanguage:(CLKLanguageType)languageType;
 ...
 *
 */
- (void)login:(CLKCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(CLKCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(CLKCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(CLKCallbackBlock)Block;
 ...
 */
- (void)userInfo:(CLKCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(CLKCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(CLKCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface CLKSDK(CLKIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(CLKCallbackBlock)block ;
